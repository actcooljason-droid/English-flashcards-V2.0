...
import { VocabularyItem, grades } from './data';

// Book Configuration Mapping
const bookConfigs: Record<string, string> = {
  '7up': '1595914256',
  '7down': '38076378',
  '8up': '1148319257',
  '8down': '1066244485',
...
const SHEET_BASE_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vSiG_-iqjLQUkpwBjDWiHJTL-b8vE0CuswFN3Jcc1fnChNd8IILJhTePxyf0nrZrAS9hhjmXECjYDbz/pub?output=csv';
...
export default function App() {
  ...
  const modules = useMemo(() => Array.from(new Set(vocabulary.map(v => v.unit))), [vocabulary]);
  ...
  const loadVocabulary = useCallback(async (bookId: string) => {
    setIsLoading(true);
    setLoadError(null);
    setProgress({}); // Clear progress for new book
    
    try {
      const gid = bookConfigs[bookId];
      ...
      Papa.parse(csvText, {
        ...
        complete: (results) => {
          ...
          const parsedData: VocabularyItem[] = dataRows
            .filter((row: any) => row.length >= 4 && row[0].trim() !== '')
            .map((row: any) => ({
              word: row[0],
              meaning: row[1],
              unit: row[2],
        ...
        error: (error: any) => {
          ...
          setLoadError('解析教材数据失败');
          setIsLoading(false);
        }
      });
    } catch (error) {
      console.error('Fetch error:', error);
      setLoadError('网络连接超时或教材加载失败');
      setIsLoading(false);
    }
  }, []);
      ...
      {isLoading ? (
        <div className="glass-card p-20 flex flex-col items-center justify-center gap-6">
          ...
          <div className="text-center">
            <p className="text-slate-500 font-medium tracking-widest mb-2">正在为您准备教材...</p>
            <p className="text-slate-400 text-xs font-light">精心挑选每一粒知识的咖啡豆</p>
          </div>
        </div>
      ) : loadError ? (
        <div className="glass-card p-12 text-center border-terracotta/20">
          <AlertCircle size={48} className="text-terracotta mx-auto mb-4" />
          <p className="text-slate-600 font-medium mb-2">{loadError}</p>
          <button 
            onClick={() => loadVocabulary(selectedGrade.id)}
            className="px-6 py-2 bg-sage text-white rounded-full text-sm hover:bg-sage-dark transition-colors"
          >
            重试加载
          </button>
        </div>
